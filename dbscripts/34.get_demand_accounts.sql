USE `adserve`;
DROP PROCEDURE IF EXISTS `get_demand_accounts`;

DELIMITER //

CREATE PROCEDURE `get_demand_accounts` (
	$id int(11),
    $account_id int(11),
    $from int,
    $size int,
...
BEGIN
    ...
    SELECT
        ...
        s.modify_date,
        s.is_active
    FROM demand_accounts s
        LEFT JOIN advertisers a ON a.demand_account_id = s.id
    WHERE 1 = 1
