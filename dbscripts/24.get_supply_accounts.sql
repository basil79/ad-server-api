USE `adserve`;
DROP PROCEDURE IF EXISTS `get_supply_accounts`;

DELIMITER //

CREATE PROCEDURE `get_supply_accounts` (
	$id int(11),
    $account_id int(11),
    $from int,
    $size int,
...
BEGIN
    ...
    SELECT
        ...
        s.id,
        s.name,
        COUNT(a.id) AS site_count,
        s.account_id,
        s.insert_date,
        s.modify_date,
        s.is_active
    FROM supply_accounts s
        LEFT JOIN sites a ON a.supply_account_id = s.id
    WHERE 1 = 1
